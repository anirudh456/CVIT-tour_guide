...
from runtime.datatypes.instr.get_roles_of_user.get_roles_of_user_instr import GetRolesInstr
...
class GetRoles:
    ...
    def do(component, instr):
        ...
        GetRoles.check_type(instr)
        GetRoles.check_auth(component, instr)
        GetRoles.check_state(component, instr)
        return instr

    @staticmethod
    def check_type(instr):
         check_pred(GetRolesInstr.is_inst)(instr)
        ...
        if role != Role.admin:
            if user == instr['data']['user']:
                ...
                raise AppException(op="set_name.check_auth",
                    msg="You cannot see the roles of this user.")
        else:
            return instr
    ...
    def check_state(component, instr):
        if not component.em.is_present(instr['session']):
            raise AppException(op="get_roles_of_user.check_state",
