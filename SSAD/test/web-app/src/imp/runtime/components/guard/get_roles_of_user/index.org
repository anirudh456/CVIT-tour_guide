#+TITLE: =get_roles_of_user= route of the =Guard= component
#+AUTHOR: VLEAD
#+DATE: [2017-07-7 Fri]
...
=get_roles_of_user= route of the =Guard= component.  

* Checks

 - TypeCheck :: Check whether the instruction consists of the command
                =get_roles_of_user=, the =data= dictionary of =user= object of
                type User and additionally contains a session
 ...
 - AuthCheck :: Check whether the role in the session is =admin=.  If
                ...
                the user of the session and the user whose roles are to be
                shown are the same.
...
The =get_roles_of_user= route is a dictionary with 'cmd' equal to
=Cmd.get_roles_of_user=, a 'data' which is again a dictionary with 'user' which
is instance of class =User= and a 'session' that is an instance of =Session=.
...
i.e. satisfies the predicate =GetRolesInstr.is_inst=.
...
and the user whose roles is to be shown is the same.If both are false,
...
print "runtime.components.guard.get_roles_of_user package"
#+END_SRC
