#+title: Test Setup for Get-user-by-email Command Handler
#+AUTHOR: VLEAD
#+DATE: [2017-07-7 Fri]
...
* Test Harness
  ...
  Instantiates a =Engine= component and populates it with the =GetUser= command
  handler.
...
tests on the =get_user_by_email= route for the engine component. 

 - creates an entity manager
 - creates a engine component with that entity manager
 - adds the =get_user_by_email='s command handler (=GetUser.do=)
...
print "runtime.components.engine.get_user_by_email.tests package"
#+END_SRC
