#+title: Test Setup for GetRoleSet Command Handler
#+AUTHOR: VLEAD
#+DATE: [2017-07-11 Tue]
...
* Test Harness
  ...
  Instantiates a =Engine= component and populates it with the =GetRoleSet= command
  handler.
...
tests on the =get_role_set_in_system= route for the engine component. 

 - creates an entity manager
 - creates a engine component with that entity manager
 - adds the =get_role_set_in_system='s command handler (=GetRoleSet.do=)
...
print "runtime.components.engine.get_role_set_in_system.tests package"
#+END_SRC
