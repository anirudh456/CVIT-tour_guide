...
** Test Case
   ...
   creates an instruction with cmd =set_email= a dictionary with an
   object that is an instance of =Emailand an existing session.  The
   engine's do method succeeds and returns the instruction and result
   of the operation as result.
