...
* Evolution of Requirements and Implementation specifications

One may think of the client presenting us a *client
requirement specification*, or client-spec, for short.  Any
implementation of the client-spec evolves as a series of
/releases/.  A release is nothing but a tag of a commit of a
branch (which, by convention, is the master branch).  Each
release has its own specification (the /release-spec/) that,
ideally, approximates the client-spec.  In the limit, the
release-specs of successive releases approach the
client-spec.  Of course, this assumes that the client-spec
itself doesn't change.  If it does, then this could widen
the gap between a release-spec and the client-spec (we are
then chasing a moving target).
...
:END: 
 ...
 1. [[./req/index.org][Requirements]]

 2. [[./imp/data-model/index.org][Data Model]]

 3. [[./imp/runtime/index.org][Runtime]]

 4. [[./imp/deployment/index.org][Deployment]]
